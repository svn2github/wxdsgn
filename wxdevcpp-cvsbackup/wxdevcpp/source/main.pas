...
type
  TMainForm = class(TForm)
    ...
    procedure actNewwxFormExecute(Sender: TObject);
...
{$IFDEF WX_BUILD}
    function CreateFormFile(strFName, strCName, strFTitle: string; dlgSStyle:TWxDlgStyleSet): Boolean;
...
procedure TMainForm.actNewwxFormExecute(Sender: TObject);
{$IFDEF WX_BUILD}
var
  NewDesigner: TEditor;
...
{$IFDEF WX_BUILD}
  ...
  begin
    MessageDlg('Unable to find wxWidgets Dlg Template files. '+#13+#10+''+#13+#10+'Please provide the template files in the template directory. ', mtError, [mbOK], 0);
    exit;
  end;
  ...
  begin
    GetIntialFormData(FCreateFormPropObj, strFName, strCName, strFTitle,
      dlgSStyle);
    CreateStatus := CreateFormFile(strFName, strCName, strFTitle, dlgSStyle);
  end;
...
{$IFDEF WX_BUILD}
function TMainForm.CreateFormFile(strFName, strCName, strFTitle: string;
  dlgSStyle: TWxDlgStyleSet): Boolean;
var
...
var
  strClassName, strClassTitle, strClassStyleString, strFileName, StrDirName,
    strDate, strAuthor: string;
  dlgStyle: TWxDlgStyleSet;
...
var
  ...
  Temp, Pos1,J: Integer;
  IniFile,PalleteIniFile: TIniFile;
  strPallList:TstringList;
...
begin
  ...
  begin
    ...
    begin
        ...
        begin
            if ELDesigner1.DesignControl.ComponentCount > 0 then
            begin
                ...
                begin
                    ...
                    MessageDlg('You cannot add a sizer if you have other standard components.'+#13+#10+''+#13+#10+'Please remove all the controls before adding a sizer.', mtError, [mbOK], 0);
                end;
            end;
        end;
    end;
...
{$IFDEF WX_BUILD}
procedure TMainForm.BuildProperties(Comp: TControl);
begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            //Item.OnGetValueList:=OnBitmapPropertyButtonClick;
          end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if CreateFunctionInEditor(Data,Trim(e.getDesigner().Wx_Name),
          SelectedComponent, str, JvInspEvents.Selected.DisplayName) then
        begin
...
{$IFDEF WX_BUILD}
function TMainForm.CreateFunctionInEditor(eventProperty:TJvCustomInspectorData;strClassName: string; SelComponent:
  TComponent; var strFunctionName: string; strEventFullName: string): Boolean;
...
{$IFDEF WX_BUILD}
function TMainForm.ReplaceClassNameInEditorFile(FileName, FromClassName,
