...
var
    MemCheckLogFileName: string = 'MemCheck.log';
...
function CallStackTextualRepresentation(const S: TCallStack; const LineHeader:
    string): string;
//Will contain CR/LFs
...
const
    ...
    CheckWipedBlocksOnTermination = True and WipeOutMemoryOnFreeMem and not
        (DanglingInterfacesVerified);
...
type
    ...
    TFieldInfo = class
        ...
        constructor Create(const TheOwnerClass: TClass; const TheFieldIndex:
            integer);
    end;
...
type
    ...
    public
        ...
        constructor Create(const AName: string; const AStartAddress: Cardinal; const
            ALength: Cardinal);
    end;
...
constructor TFieldInfo.Create(const TheOwnerClass: TClass; const TheFieldIndex:
    integer);
begin
    inherited Create;
...
begin
    ...
    try
        ...
        FeedBackStr := FeedBackStr + ' on a FREED instance of ' + T.VMT.ClassName +
            ' (destroyed at ' +
            TextualDebugInfoForAddress(Cardinal(T.DestructionAdress))
            + ' - had been created at ' +
    ...
    if ShowCallStack then
        for i := 1 to StoredCallStackDepth do
            ...
            try
                raise EStackUnwinding.Create('Unwinding level ' + chr(ord('0') + i))at
                    T.CallerAddress[i]
            except
...
begin
    try
        ...
        raise
            EInterfaceFreedInstance.Create('Calling an interface method on an freed Pascal instance')at InterfaceErrorCaller
    except
...
begin
    ...
    Count := 0;
    SkipCount := 0;
    while (longint(CurrentFrame) >= longint(StackStart)) and (longint(CurrentFrame)
        < longint(StackMax)) and (Count <= StoredCallStackDepth) do
    begin
        ...
        begin
            try
                OutputFileHeader := OutputFileHeader +
                    ...
                    ' - Already freed at ' +
                    TextualDebugInfoForAddress(Cardinal(Block.DestructionAdress)) +
                    #13#10;
                raise
                    EMemoryLeak.Create('second release of block attempt, already freed')at
                    Block.DestructionAdress;
            except
                on EMemoryLeak do
                    ;
            end;
            ...
            if ShowCallStack then
                for i := 1 to StoredCallStackDepth do
                    ...
                    try
                        raise EStackUnwinding.Create('Unwinding level ' + chr(ord('0') +
                            i))at Block.CallerAddress[i]
                    except
                        on EStackUnwinding do
                            ;
                    end;
            ...
            begin
                try
                    OutputFileHeader := OutputFileHeader +
                        ...
                        #13#10;
                    raise
                        EMemoryLeak.Create('memory damaged beyond block allocated space, allocated at '
                        + TextualDebugInfoForAddress(Cardinal(BlockAllocationAddress(P))))at
                        CallerOfCaller;
                except
                    on EMemoryLeak do
                        ;
...
begin
    ...
    begin
        ...
        if Size > PMemoryBlocHeader(PChar(P) -
            SizeOf(TMemoryBlocHeader)).AllocatedSize then
            Move(P^, Result^, PMemoryBlocHeader(PChar(P) -
                SizeOf(TMemoryBlocHeader)).AllocatedSize)
        else
            Move(P^, Result^, Size);
    ...
    begin
        ...
        begin
            ...
            begin
                ...
                if (BlocksToShow[Block.KindOfBlock]) and ((Block.KindOfBlock <> MClass)
                    or (TObject(PChar(Block) + SizeOf(TMemoryBlocHeader)) is
                    InstancesConformingToForReporting)) then
                try
                    raise EMemoryLeak.Create(S + ' allocated at ' +
                        TextualDebugInfoForAddress(Cardinal(Block.CallerAddress[0])))at
                        Block.CallerAddress[0];
                except
                    on EMemoryLeak do
                        ;
                end;
                ...
                if ShowCallStack then
                    for i := 1 to StoredCallStackDepth do
                        if Integer(Block.CallerAddress[i]) > 0 then
                        try
                            raise EStackUnwinding.Create(S + ' unwinding level ' + chr(ord('0')
                                + i))at Block.CallerAddress[i]
                        except
                            on EStackUnwinding do
                                ;
            ...
            else if CheckWipedBlocksOnTermination and
                (Block.AllocatedSize > 5) and (Block.AllocatedSize <=
                DoNotCheckWipedBlocksBiggerThan) and (not
                IsMemFilledWithChar(pchar(Block) + SizeOf(TMemoryBlocHeader) + 4,
                Block.AllocatedSize - 5, CharToUseToWipeOut)) then
            begin
                try
                    raise
                        EFreedBlockDamaged.Create('Destroyed block damaged - Block allocated at ' +
                        TextualDebugInfoForAddress(Cardinal(Block.CallerAddress[0])) +
                        ' - destroyed at ' +
                        TextualDebugInfoForAddress(Cardinal(Block.DestructionAdress)))at
                        Block.CallerAddress[0]
                except
...
{$IFNDEF DELPHI7_OR_LATER}
    UnitsInfo := PackageInfo(pointer(Pointer(PChar(@AllocMemSize) + 31 * 4 +
        8)^));
    //Hacky, no ? I learnt to count on my fingers ! (this stuff is not exported by system.pas)
{$ELSE}
...
{$ENDIF}
    ...
    begin
        ...
        else if {$IFNDEF DELPHI6_OR_LATER}@{$ENDIF}CurrentUnitInfo.Init =
        @SysUtils.SysUtils then
            NewUnitsInfoOrder.Insert(1, CurrentUnitInfoCopy)
        else
{$IFDEF DELPHI6_OR_LATER}if CurrentUnitInfo.Init = @Variants.Variants then
                NewUnitsInfoOrder.Insert(2, CurrentUnitInfoCopy)
            else
{$ENDIF}if {$IFNDEF DELPHI6_OR_LATER}@{$ENDIF}CurrentUnitInfo.Init =
                Pointer(PChar(@Dummy) + DummyToFinalizationOffset) then
    ...
    begin
        WriteProcessMemory(ProcessHandle, Pointer(PChar(@UnitsInfo^.UnitInfo^[0]) + i
            * SizeOf(PackageUnitEntry)), NewUnitsInfoOrder[i],
            SizeOf(PackageUnitEntry),
            BytesWritten);
...
begin
    ...
    begin
        ...
        if TUnitDebugInfos(Units[Pivot]).Addresses[0].Address > Address then
            ...
            TUnitDebugInfos(Units[Pivot]).Addresses[Length(TUnitDebugInfos(Units[Pivot]).Addresses) - 1].Address < Address then
            Start := Pivot + 1
        else
        begin
            Result := Units[Pivot];
...
constructor TUnitDebugInfos.Create(const AName: string; const NbLines:
    Cardinal);
begin
    Name := AName;
...
constructor TRoutineDebugInfos.Create(const AName: string; const AStartAddress:
    Cardinal; const ALength: Cardinal);
begin
    Name := AName;
...
function TUnitDebugInfos.LineWhichContainsAddress(const Address: Cardinal):
    string;
var
...
procedure dumpsymbols(NameTbl: PArrayOfPointer; sstptr: PArrayOfByte; size:
    integer);
...
procedure dumplines(NameTbl: PArrayOfPointer; sstptr: PArrayOfByte; size:
    integer);
//Copyright (C) Tenth Planet Software Intl., Clive Turvey 1998. All rights reserved. - Reused & modified by SG with permission
var
    ...
    begin
        ...
        begin
            ...
            begin
                ...
                CurrentUnit :=
                    TUnitDebugInfos.Create(ExtractFileName(PChar(NameTbl^[srcfile^._nName
                    -
                        1])), srcln^._cPair);
                AddUnit(CurrentUnit);
                ...
                for k := 0 to pred(srcln^._cPair) do
                    CurrentUnit.Addresses[k] :=
                        TAddressToLine.Create(Integer(PArrayOfPointer(@srcln^._Offset[0])^[k]), Integer(PArrayOfWord(@srcln^._Offset[srcln^._cPair])^[k]));
            end;
        end;
    end;
end;
...
begin
    ...
    begin
        SetDispl;
        OutOfMemory :=
            EOutOfMemory.Create('Memcheck is not able to allocate memory, due to system resource lack');
...
begin
    ...
    if fBlock.KindOfBlock = MClass then
        Result := Result and (TObject(PChar(fBlock) +
            SizeOf(TMemoryBlocHeader)).ClassName = TObject(PChar(Other.Block) +
            SizeOf(TMemoryBlocHeader)).ClassName);

    Result := Result and (WasFieldOfAnotherObject =
        Other.WasFieldOfAnotherObject);
...
procedure GetLeaks(const LeaksList, ChronogicalInfo: TLeakList; const
    MaxNumberOfLeaks: integer; var StoppedDueToMaxLeak: Boolean);
var
    Block: PMemoryBlocHeader;
    CurrentLeak: TLeak;
    i: integer;
...
procedure GetBadBlocks(const B: TBlockList; const MaxNumberOfBlocks,
    MaxBlockSize: integer; var StoppedDueToMaxBlock: Boolean);
var
    Block: PMemoryBlocHeader;
begin
    ...
    begin
        if MemoryBlockFreed(Block) and (Block.AllocatedSize > 5) and
            (Block.AllocatedSize <= MaxBlockSize) and (not
            IsMemFilledWithChar(pchar(Block) + SizeOf(TMemoryBlocHeader) + 4,
            Block.AllocatedSize - 5, CharToUseToWipeOut)) then
            B.Add(Block);
...
begin
    ...
    begin
        ...
        begin
            for j := LastDisplayedTimeStamp + 1 to
                ChronogicalInfo.Item(i).Block.LastTimeStamp do
    ...
    begin
        ...
        begin
            ...
            Write(OutputFile,
                CallStackTextualRepresentation(BadBlocks.Item(i).CallerAddress,
                #9#9#9));
    ...
    if ShowLogFileWhenUseful and (LeaksList.Count > 0) or
        CollectStatsAboutObjectAllocation or ComputeMemoryUsageStats or
        KeepMaxMemoryUsage then
...
begin
    ...
    NTOptHeader := PImageOptionalHeader(Cardinal(NTHeader) +
...
begin
    ...
    begin
        ...
        if U <> nil then
            Result := 'Module ' + U.Name +
                RoutineWhichContainsAddress(AddressInDebugInfos) +
