...
{$ENDIF}
    ...
    function PrevWordPosForDelete: TPoint; virtual;
...
// START: mandrav - 5 Jul 2002
  fMouseDownX := X;
  fMouseDownY := Y;
// END: mandrav - 5 Jul 2002
...
//  fLastCaretX := fCaretX;                                                     //mh 2000-10-19
    fLastCaretX := DisplayX;                                              //sblbg 2001-12-19
  end;
...
{ local procedures }
  ...
  begin
    ...
    begin
      ...
      begin
        if (fShowSpecChar) and (Token[Counter] = ' ') then Token[Counter] := SynSpaceGlyph;
        if (Token[Counter] = TSynTabChar) then     // plpolak
        begin
          Token[Counter] := ' ';  //Tabs painted differently if necessary
      ...
      begin
        //fix everything before the FirstChar
          for i := 1 to First - 1 do               //wipe the text out so we don't
            if sTabbedToken[i] = TSynTabChar then  //count it out of the range
              sTabbedToken[i] := ' ';              //we're looking for
        ...
        begin
          ...
          for i := 0 to TabLen - 1 do           //wipe the text out so we don't
            sTabbedToken[TabStart + i] := ' ';  //count it again
...
{$ENDIF}
  ...
  procedure InsertText;
    ...
    begin
      ...
      if eoTrimTrailingSpaces in Options then
        fCaretX := 1 + Length(Lines[CaretY - 1]) - Length(TrimRight(sRightSide))
...
procedure TCustomSynEdit.InsertBlockEx(BB, BE: TPoint; ChangeStr: PChar;
...
{end}                                                                           //sbs 2000-11-20
      crIndent:
        begin
          ...
          SetCaretAndSelection(Item.ChangeEndPos,
            Item.ChangeStartPos, Item.ChangeEndPos);
        end;
      crUnindent :
...
// word selection
      ecWordLeft, ecSelWordLeft:
        begin
          ...
          //GBN 13/11/2001
          if (CaretNew.Y<Caret.Y) and (Caret.X<length(Lines[CaretNew.Y-1])) then
            begin
            CaretNew.X:=length(Lines[CaretNew.Y-1])+1;
            end;
          //End GBN 13/11/2001
...
//mh 2000-10-19
      ...
      ecWordRight, ecSelWordRight:
        begin
          ...
          //GBN 13/11/2001
          if (CaretNew.Y>Caret.Y) and (Caret.X<length(LineText)) then
            begin
            CaretNew.X:=length(LineText)+1;
            CaretNew.Y:=CaretY;
            end;
          //End GBN 13/11/2001
            ...
            if CaretX > Len + 1 then begin
              ...
              begin
                ...
                if Len > 0 then
                  ...
                  if SpaceCount2 = SpaceCount1 then
                    SpaceCount2 := 0;
                  CaretX := fCaretX - (SpaceCount1 - SpaceCount2);              // CL: was fCaretX :=, so didn't scroll
//                  fLastCaretX := fCaretX;
                  fLastCaretX := DisplayX;                                      //sblbg 2001-12-19
...
{begin}                                                                         //Fiala
          if fWordWrap then begin
            if (Length(Lines[CaretY-1]) < CharsInWindow) or WasLineJoin
            then begin
...
{end}                                                                           //mh 2000-11-20
            ...
            if Len < CaretX then
            begin
              SpaceBuffer := StringOfChar(' ', CaretX - Len - Ord(fInserting));
...
function TCustomSynEdit.PrevWordPosForDelete: TPoint;
var
  CX, CY, MultiPos: integer;
  Line: string;
  IdentChars, WhiteChars: TSynIdentChars;
...
{$ELSE}
    ...
    if Line[CX - 1] in WhiteChars then
    begin
      CX := StrRScanForCharInSet(Line, CX - 1, IdentChars) + 1;
      while (CX>1) and (Line[CX - 1] in IdentChars) do
        Dec(CX);
    end
    else
{$ENDIF}
...
begin
 ...
 { CaretX := value - count;
  ...
  CaretXY:= Point(value -count, loop +1);
  BlockBegin:= CaretXY;
end;
...
begin
  ...
  try
   case cmd of
    ecUpperCase, ecLowerCase, ecTitleCase:
     begin
       w:= WordAtCursor;
       if cmd = ecUpperCase then
        w:= AnsiUpperCase(w)
       else
        if cmd = ecLowerCase then
         w:= AnsiLowerCase(w)
        else
     ...
     begin
       ...
       if SelText = '' then
        begin
          bNoSel:= True;
          SetBlockBegin(CaretXY);
          SetBlockEnd(point(CaretX +1, CaretY +1));
...
{begin}                                                                         //Fiala
        if fWordWrap then
          while (iLine > 0) and TSynEditStringList(fLines).IsLineWraped(iLine)
          do begin
            Dec(iLine);
...
{end}                                                                           //Fiala
        if (Length(PrevLine) >= MinLen) then begin
          p := @PrevLine[1];
          repeat
            if not(p^ in [#32, #9]) then break;
            Inc(i);
            Inc(p);
          until (p^ = #0) or (i>= MinLen);
          if (i>= MinLen) then
          begin
...
{begin}                                                                         //Fiala
    ...
    then Spaces := StringOfChar(' ', i)
      else Spaces := TSynTabChar;
    end else begin
     Spaces:= CopyString;
    end;
{end}                                                                           //Fiala
    NewCaretX := StartOfBlock.X + length(Spaces);
    fOverrideTrim:= Pos(TSynTabChar, Spaces) <> 0;
    SetSelText(Spaces);
    fOverrideTrim:= FALSE;
...
begin
  ...
  try
    ...
    fUndoList.AddChange(crIndent, Point(BB.x +fTabWidth, BB.y), point(x, BB.y), '', smColumn);
    fUndoList.EndBlock;
  finally
...
begin
  if (eoTrimTrailingSpaces in Options) and not(fOverrideTrim) then
