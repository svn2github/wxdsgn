...
begin
  if Assigned(Source) and (Source is TSynEdit) then
  begin
    Self.Font.Assign(TSynEdit(Source).Font);
    Self.BookmarkOptions.Assign(TSynEdit(Source).BookmarkOptions);
    Self.Gutter.Assign(TSynEdit(Source).Gutter);
    Self.Keystrokes.Assign(TSynEdit(Source).Keystrokes);
    Self.SelectedColor.Assign(TSynEdit(Source).SelectedColor);

    Self.Color := TSynEdit(Source).Color;
    Self.Options := TSynEdit(Source).Options;
    Self.ExtraLineSpacing := TSynEdit(Source).ExtraLineSpacing;
    Self.HideSelection := TSynEdit(Source).HideSelection;
    Self.InsertCaret := TSynEdit(Source).InsertCaret;
    Self.OverwriteCaret := TSynEdit(Source).OverwriteCaret;
    Self.MaxLeftChar := TSynEdit(Source).MaxLeftChar;
    Self.MaxUndo := TSynEdit(Source).MaxUndo;
    Self.RightEdge := TSynEdit(Source).RightEdge;
    Self.RightEdgeColor := TSynEdit(Source).RightEdgeColor;
    Self.TabWidth := TSynEdit(Source).TabWidth;
    Self.WantTabs := TSynEdit(Source).WantTabs;
...
begin
  if Assigned(Dest) and (Dest is TSynEdit) then
  begin
    TSynEdit(Dest).Font.Assign(Self.Font);
    TSynEdit(Dest).BookmarkOptions.Assign(Self.BookmarkOptions);
    TSynEdit(Dest).Gutter.Assign(Self.Gutter);
    TSynEdit(Dest).Keystrokes.Assign(Self.Keystrokes);
    TSynEdit(Dest).SelectedColor.Assign(Self.SelectedColor);

    TSynEdit(Dest).Color := Self.Color;
    TSynEdit(Dest).Options := Self.Options;
    TSynEdit(Dest).ExtraLineSpacing := Self.ExtraLineSpacing;
    TSynEdit(Dest).HideSelection := Self.HideSelection;
    TSynEdit(Dest).InsertCaret := Self.InsertCaret;
    TSynEdit(Dest).OverwriteCaret := Self.OverwriteCaret;
    TSynEdit(Dest).MaxLeftChar := Self.MaxLeftChar;
    TSynEdit(Dest).MaxUndo := Self.MaxUndo;
    TSynEdit(Dest).RightEdge := Self.RightEdge;
    TSynEdit(Dest).RightEdgeColor := Self.RightEdgeColor;
    TSynEdit(Dest).TabWidth := Self.TabWidth;
    TSynEdit(Dest).WantTabs := Self.WantTabs;
  end else
    inherited;
end;
...
initialization
  ...
  Classes.RegisterClass(TSynEditorOptionsContainer);
