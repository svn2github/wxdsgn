      ...
      if Assigned(Attr) then begin
        ...
        if FColors then begin
          ...
          if AColor = clNone then
            ...
            if AColor = fDefaultBG then
              AColor := AColor xor $00FFFFFF;
      ...
      for i := TPageLine(FPages[Num - 1]).FirstLine to iEnd do begin
        ...
        if (not fSelectedOnly or ((i >= fBlockBegin.Y - 1) and (i <= fBlockEnd.Y - 1))) then begin
          if (not fSelectedOnly or (fSelMode = smLine)) then WriteLine(Lines[i])
            // this bug fix just doesn't honor the selstart and selend...
            // it prints the whole of the starting and ending lines,
            // but that's a minor problem compared to before, because it didn't
            // print anything ;)
