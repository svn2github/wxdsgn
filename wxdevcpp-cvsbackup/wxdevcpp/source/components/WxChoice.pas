...
type
  TWxChoice = class(TComboBox,IWxComponentInterface,IWxToolBarInsertableInterface,IWxToolBarNonInsertableInterface)
    private
      { Private fields of TWxChoice }
        ...
        FWx_Class : String;
        { Storage for property Wx_ChoiceStyle }
//        FWx_ChoiceStyle : TWxChcStyleSet;
...
begin
     ...
     FWx_PropertyList.add('Wx_ChoiceStyle:Choice Style');
...
begin
     ...
     begin
          Result:=Format('EVT_CHOICE(%s,%s::%s)',[WX_IDName,CurrClassName,EVT_CHOICE]) +'';
     end;
...
//    strStyle:=GetcomboBoxSpecificStyle(Wx_GeneralStyle,Wx_ComboboxStyle);
    ...
    begin
        Result:=Result +#13+Format('%s.Add(%s);',['arrayStringFor_'+self.Name,GetCppString(self.Items[i])]);
    end;
    ...
    Result:=Result +#13+Format('%s = new %s(%s, %s, wxPoint(%d,%d), wxSize(%d,%d), %s%s);',[self.Name,self.Wx_Class,ParentName,GetWxIDString(self.Wx_IDName,self.Wx_IDValue),self.Left,self.Top,self.width,self.Height,'arrayStringFor_'+self.Name,strStyle] );
